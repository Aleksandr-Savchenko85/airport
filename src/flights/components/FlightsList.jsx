...
const FlightsList = ({ flights, isFetching }) => {
    ...
    const fligtsListDirectionMapped = flightsToday.map(el => {
        ...
        let timeShedule = el.status === "DP"
            ? moment(el.timeDepShedule).format("HH:mm") : el.status === "LN"
                ? moment(el.timeArrShedule).format("HH:mm")
                : moment(el.timeToStand).format("HH:mm");
    ...
    //console.log(fligtsListDirectionMapped)
